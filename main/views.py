...
def product_detail(request,slug,id):
	...
	# Check
	canAdd=True
	reviewCheck=ProductReview.objects.filter(user=request.user.id,product=product).count()
	...
	return render(request,'product_detail.html',{'data':product,'related':related_products,'colors':colors,'sizes':sizes,'form':reviewForm,'canAdd':canAdd,'reviews':reviews,'avg_reviews':avg_reviews})
...
def checkout(request):
	...
	if 'cartdata' in request.session:
		...
		paypal_dict = {
		    ...
		    'amount': total_amt*0.012,  # INR 1 = USD 0.012
...
def my_orders(request):
	orders=CartOrder.objects.filter(user=request.user.id).order_by('-id')
...
def my_wishlist(request):
	wlist=Wishlist.objects.filter(user=request.user.id).order_by('-id')
	return render(request, 'user/wishlist.html',{'wlist':wlist})

# My Reviews
def my_reviews(request):
	reviews=ProductReview.objects.filter(user=request.user.id).order_by('-id')
	return render(request, 'user/reviews.html',{'reviews':reviews})

# My AddressBook
def my_addressbook(request):
	addbook=UserAddressBook.objects.filter(user=request.user.id).order_by('-id')
